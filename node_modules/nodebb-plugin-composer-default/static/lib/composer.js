...
define('composer', [
	...
	composer.newTopic = async (data) => {
		...
		console.log('New Topic postObj:', pushData); // Print the postObj
		push(pushData);
	};
	...
	composer.newReply = function (data) {
		translator.translate(data.body, config.defaultLang, function (translated) {
			push({
				...
				isMain: false,
				isAnonymous: data.isAnonymous || false, // Include the isAnonymous field
			});
		});
	};
	...
	composer.enhance = function (postContainer, post_uuid, postData) {
		...
		*/
		...
		postContainer.on('change', '#anonymousToggle', function () {
			...
			console.log("Before change:", composer.posts[post_uuid].isAnonymous);
			isAnonymous = $(this).is(':checked');
			console.log("After change:", composer.posts[post_uuid].isAnonymous);
		});
	...
	async function createNewComposer(post_uuid) {
		...
		var data = {
			...
			isAnonymous: config.allowAnonymousPosts && app.user.isAnonymous,
			submitOptions: [
