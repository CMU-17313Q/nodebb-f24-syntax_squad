...
<!-- Including a unique data-pid attribute for each post -->
<div class="post-container d-flex flex-grow-1 flex-column w-100" style="min-width:0;" data-pid="{posts.pid}" data-best="{posts.best}">
	<!-- Best Response Mark (Initially Hidden) -->
	<div class="best-response-text" style="color: green; font-weight: bold; display: none;">
		<p>Post Marked as Best Response</p>
	</div>
</div>


<!-- JavaScript to control visibility of best response mark based on `data-best` attribute -->
<script>
	document.addEventListener("DOMContentLoaded", function() {
		...
		postContainers.forEach(function(postContainer) {
			// Get the value of `data-best` attribute, which will be "true" or "false"
			const isBestResponse = postContainer.getAttribute('data-best') === "true";

			// Select the best response text div inside the current post container
			const bestResponseText = postContainer.querySelector('.best-response-text');

			// Show or hide the best response text based on the value of `data-best`
			if (isBestResponse && bestResponseText) {
				bestResponseText.style.display = 'block';  // Show best response
			} else if (bestResponseText) {
				bestResponseText.style.display = 'none';  // Hide best response
			}
		});
	});
</script>
	...
	<div component="post/actions" class="d-flex justify-content-end gap-1 post-tools">
		...
		<!-- IMPORT partials/topic/emoji-dropdown.tpl -->
